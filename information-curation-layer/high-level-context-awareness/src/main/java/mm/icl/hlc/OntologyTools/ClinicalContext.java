...
/**
 * clinical Context is the class encapsulating the OntModel associated to a
 * clinical Context Instance. This is an instance of the clinical Context
 ...
 * @since 2015-10-28
 */
public class ClinicalContext extends Context{   
	/**
	 * Constructor of a new clinical Context instance.
	 * 
	 * @param hlcModel
	 *            OntModel associated to the clinical Context instance.
	 */
	public ClinicalContext(OntModel hlcModel) {
		super(hlcModel);
	}
	/**
	 * Constructor of a new clinical Context instance.
	 ...
	 *            Model.
	 */
	public ClinicalContext(OntModel hlcModel, ContextOntology ont) {
		super(hlcModel);
		OntModel hlcInst = ModelFactory.createOntologyModel(OntModelSpec.OWL_DL_MEM, hlcModel);
		hlcInst.addSubModel(ont.getCtxModel());
		ExtendedIterator<? extends OntResource> it = hlcInst.getOntClass(HLCA.clinicalClassName).listInstances(false); 
		if (it.hasNext()) {
			...
			if (!it.hasNext()) {
				...
				while (iter.hasNext()) {
					...
					if (uri != null && !uri.equals(OWL2.NamedIndividual.getURI())) {
						if (!uri.equals(HLCA.clinicalClassName)) {   
	...
	/**
	 * Method to retrieve whether a clinical Context instance is valid or not.
	 * 
	 * @return true if the clinical Context Instance is valid, and false
	 *         otherwise.
	 */
	...
	/**
	 * Method to retrieve whether a clinical Context instance has been
	 * classified or not.
	 * 
	 * @return true if the clinical Context Instance is classified, and false
	 *         otherwise.
	 */
	...
	/**
	 * Method to set the clinical Context Instance.
	 * 
	 * @param inst
	 *            OntResource representing the clinical Context Instance.
	 */
	public void setHlcInstance(OntResource inst) {
		setCtxInstanceName(inst.getURI());
	}
	/**
	 * Method to set the class to which a the clinical Context Instance
	 * belongs to.
	 * 
	 * @param c
	 *            OntClass to which the clinical Context Instance belongs and
	...
	/**
	 * Method to set that the clinical Context Instance does not belongs to
	 * any of the defined classes in the Context Ontology. This process does not
	 * involve any changes in the OntModel associated to the clinical Context
