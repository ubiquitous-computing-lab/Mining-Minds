...
//		ElementTriplesBlock block = createTriplesBlockForHlcUserStartTime(
	 ...
	 *         High Level Context instance.
	 */
	public static Query generateQueryForPreviousValidAndUnfinishedHlc(NutritionContext hlc, ContextOntology ont) {
...
//		ElementTriplesBlock block = createTriplesBlockForHlcUserStartAndEndTime(
	 ...
	 *         Level Context instance.
	 */
	public static Query generateQueryForPreviousValidAndFinishedHlc(NutritionContext hlc, ContextOntology ont) {
...
//		ElementTriplesBlock block = createTriplesBlockForHlcUserStartTime(hlc.getObjectPropertyValue(ont.getContextOfProp()), ont);
		ElementUnion union= createTriplesUnionForHlcUserStartTime(hlc.getObjectPropertyValue(ont.getContextOfProp()), ont); //Asif MMV2.5
	 ...
	 *         Level Context instance.
	 */
	public static Query generateQueryForStartTimeOfNextHlc(NutritionContext hlc, ContextOntology ont) {
...
//		ElementTriplesBlock block = createTriplesBlockForLlcCategoryUserStartTime(
/*		
		System.out.println("CQG");
//		System.out.println(llc.getClass());            (LowLevelContext)
		
		System.out.println (ont.getLlcTypeClass(llc.getLlcCategoryName()));        ///grain, in case of rice (it should be Food), and activity in case of sitting
		System.out.println (llc.getObjectPropertyValue(ont.getContextOfProp()));
				
		for (Iterator i = ont.getLlcTypeClass(llc.getLlcCategoryName()).listSuperClasses(true); i.hasNext();)       //Lists the Super Classes
		
		{
			 OntClass c = (OntClass) i.next();
			 
		// System.out.print("The SuperClasss is  "+i+" i " + c.getLocalName().toUpperCase() + "\n");
		...
		ElementUnion union= createTriplesUnionForLlcCategoryUserStartTime (ont.getLlcTypeClass(llc.getLlcCategoryName()), 
																			llc.getObjectPropertyValue(ont.getContextOfProp()), 	ont);//Asif MMv2.5
	...
	public static Query generateQueryForPreviousValidAndUnfinishedLlc(OntClass category, Resource user, Literal time,
		...
		ElementUnion union = createTriplesUnionForLlcCategoryUserStartTime(category, user, ont); //Asif MMV2.5
...
//		ElementTriplesBlock block = createTriplesBlockForLlcCategoryUserStartAndEndTime(
		ElementUnion union = createTriplesUnionForLlcCategoryUserStartAndEndTime( //Asif MMV2.5
	...
	public static Query generateQueryForPreviousValidAndFinishedLlc(OntClass category, Resource user, Literal time,
		...
		//ElementTriplesBlock block = createTriplesBlockForLlcCategoryUserStartAndEndTime(category, user, ont);
		ElementUnion union = createTriplesUnionForLlcCategoryUserStartAndEndTime(category, user, ont); //Asif MMV2.5
...
//		ElementTriplesBlock block = createTriplesBlockForLlcCategoryUserStartTime(
		ElementUnion union = createTriplesUnionForLlcCategoryUserStartTime( //Asif MMV2.5
				ont.getLlcTypeClass(llc.getLlcCategoryName()), llc.getObjectPropertyValue(ont.getContextOfProp()), ont);
	 ...
	 */
	//public static Query generateQueryForLlcWithEndTimeInWindowAndNoContiguousLlc(Literal windowStart, Literal windowEnd, ContextOntology ont) {
	
	
	public static Query generateQueryForLlcWithEndTimeInWindowAndNoContiguousLlc(Literal windowStart, Literal windowEnd, ContextOntology ont) { //Asif MMV2.5


//		ElementTriplesBlock block = createTriplesBlockForLlcUserAndEndTimeVar(ont);
...
//		ElementTriplesBlock block = createTriplesBlockForLlcUserStartTime(
		
		
		ElementUnion union = createTriplesUnionForLlcUserStartTime(      //Asif MMV2.5
...
//		ElementTriplesBlock block = createTriplesBlockForLlcUserStartAndEndTime(
				ElementUnion union = createTriplesUnionForLlcUserStartAndEndTime(       //Asif MMV2.5
				llc.getObjectPropertyValue(ont.getContextOfProp()), ont);
...
//	private static ElementTriplesBlock createTriplesBlockForHlcUserStartTime(Resource user, ContextOntology ont) {
		private static ElementUnion createTriplesUnionForHlcUserStartTime(Resource user, ContextOntology ont) { //Asif
...
/*		Triple pattern4 = Triple.create(Var.alloc(HLCA.hlcSparqlVar), Nodes.type, ont.getNutHlcClass().asNode());           //////// MMV2.5 for NutritionContext
...
//		System.out.println("block 1  " + union);
		return union;
	}
		 ...
		 */
//		private static ElementTriplesBlock createTriplesBlockForHlcUserStartTime(Resource user, ContextOntology ont) {
			private static ElementUnion createTriplesUnionForNutHlcUserStartTime(Resource user, ContextOntology ont) { //Asif
/*
			Triple pattern1 = Triple.create(Var.alloc(HLCA.hlcSparqlVar), Nodes.type, ont.getHlcClass().asNode());
			Triple pattern2 = Triple.create(Var.alloc(HLCA.hlcSparqlVar), ont.getContextOfProp().asNode(), user.asNode());
			Triple pattern3 = Triple.create(Var.alloc(HLCA.hlcSparqlVar), ont.getStartTimeProp().asNode(),
...
//		private static ElementTriplesBlock createTriplesBlockForHlcUserStartAndEndTime(Resource user, ContextOntology ont) {
		private static ElementUnion createTriplesUnionForHlcUserStartAndEndTime(Resource user, ContextOntology ont) {//Asif MMV2.5
...
//		Triple pattern2 = Triple.create(Var.alloc(HLCA.hlcSparqlVar), ont.getEndTimeProp().asNode(),Var.alloc(HLCA.endTimeSparqlVar));
...
//		System.out.println("block 2  " + block);
	...
	//CHANGE REQUIRED FOR QEUERY HERE        LESS USED
//	private static ElementTriplesBlock createTriplesBlockForLlcCategoryUserStartTime(OntClass category, Resource user,
	private static ElementUnion createTriplesUnionForLlcCategoryUserStartTime(OntClass category, Resource user,  //Asif MMV2.5
		...
		OntClass SuperClassCategory  = ont.getLlcSuperClassCategory(category);		//It Finds the SuperClass for Category Needed for Finding SuperClass of All Food Items (Asif MMV2.5)
	
//		System.out.println("I am in CQG Line 609:"+ SuperClassCategory.asNode() );
		
		
/*		This has been Moved to ContextOntology for Structuring.
...
//				System.out.println(" Outside loop : "+ c.getURI());
//				System.out.println(" Outside loop : "+ c.asNode());
				ElementTriplesBlock blocka = new ElementTriplesBlock();
				ElementTriplesBlock blockb = new ElementTriplesBlock();
				...
				{
//					System.out.println("This is perfect for Food");
				pattern1 = Triple.create(Var.alloc(HLCA.llcSparqlVar), Nodes.type, Var.alloc(HLCA.llcTypeSparqlVar));
				pattern2 = Triple.create(Var.alloc(HLCA.llcTypeSparqlVar),com.hp.hpl.jena.vocabulary.RDFS.Nodes.subClassOf, SuperClassCategory.asNode());   //here
				...
				pattern7 = Triple.create(Var.alloc(HLCA.llcType1SparqlVar),com.hp.hpl.jena.vocabulary.RDFS.Nodes.subClassOf, SuperClassCategory.asNode()); //here
...
//					System.out.println("Try other one for other Activities");
				
				pattern1 = Triple.create(Var.alloc(HLCA.llcSparqlVar), Nodes.type, Var.alloc(HLCA.llcTypeSparqlVar));
				pattern2 = Triple.create(Var.alloc(HLCA.llcTypeSparqlVar),com.hp.hpl.jena.vocabulary.RDFS.Nodes.subClassOf, category.asNode());   //here
				pattern3 = Triple.create(Var.alloc(HLCA.llcSparqlVar), ont.getContextOfProp().asNode(), user.asNode());
				pattern4 = Triple.create(Var.alloc(HLCA.llcSparqlVar), ont.getStartTimeProp().asNode(),Var.alloc(HLCA.startTimeSparqlVar));
				...
				pattern5 = Triple.create(Var.alloc(HLCA.llcSparqlVar), Nodes.type, Var.alloc(HLCA.llcTypeSparqlVar));
				pattern6 = Triple.create(Var.alloc(HLCA.llcTypeSparqlVar),com.hp.hpl.jena.vocabulary.RDFS.Nodes.subClassOf, Var.alloc(HLCA.llcType1SparqlVar));
				pattern7 = Triple.create(Var.alloc(HLCA.llcType1SparqlVar),com.hp.hpl.jena.vocabulary.RDFS.Nodes.subClassOf, category.asNode()); //here
				pattern8 = Triple.create(Var.alloc(HLCA.llcSparqlVar), ont.getContextOfProp().asNode(), user.asNode());
				pattern9 = Triple.create(Var.alloc(HLCA.llcSparqlVar), ont.getStartTimeProp().asNode(),Var.alloc(HLCA.startTimeSparqlVar));
				}
...
/*				
		...
		Triple pattern2 = Triple.create(Var.alloc(HLCA.llcTypeSparqlVar),com.hp.hpl.jena.vocabulary.RDFS.Nodes.subClassOf, category.asNode());   //here
		...
		Triple pattern7 = Triple.create(Var.alloc(HLCA.llcType1SparqlVar),com.hp.hpl.jena.vocabulary.RDFS.Nodes.subClassOf, category.asNode()); //here
...
/*		System.out.println("CQG 1 2 Line 635 " );
		System.out.println( category.asNode());  //SeaFood*/
...
/*System.out.println("CQG Line 702" );
...
*/
	...
	//CHANGED COMPLETELY ASIF MMV2.5
	
//	private static ElementTriplesBlock createTriplesBlockForLlcCategoryUserStartAndEndTime(OntClass category,
	private static ElementUnion createTriplesUnionForLlcCategoryUserStartAndEndTime(OntClass category,//Asif MM V2.5
...
//		Triple patternb = Triple.create(Var.alloc(HLCA.llcSparqlVar), ont.getEndTimeProp().asNode(), Var.alloc(HLCA.endTimeSparqlVar));
		
		ElementTriplesBlock blocka = new ElementTriplesBlock();
//		ElementTriplesBlock blockb = new ElementTriplesBlock();
		blocka.addTriple(patterna);
//		blockb.addTriple(patternb);
...
//		union.addElement(blockb);
	...
	//CHANGE REQUIRED FOR QEUERY UPDATION HERE
//	private static ElementTriplesBlock createTriplesBlockForLlcStartTime(ContextOntology ont) {
	private static ElementUnion createTriplesUnionForLlcStartTime(ContextOntology ont) { //Asif MMV2.5
...
/*		
			System.out.println("Line 827 Printing block 5 ") ;
			System.out.println( union);
...
*/			
	...
	//CHANGED TO MAKE IN UNION SHAPE
//	private static ElementTriplesBlock createTriplesBlockForLlcStartAndEndTime(ContextOntology ont) {
	private static ElementUnion createTriplesUnionForLlcStartAndEndTime(ContextOntology ont) { //Asif MV2.5

//		ElementTriplesBlock block = createTriplesBlockForLlcStartTime(ont);
		ElementUnion union = createTriplesUnionForLlcStartTime(ont);// Asif MMV2.5
		
		
		Triple pattern1 = Triple.create(Var.alloc(HLCA.llcSparqlVar), ont.getEndTimeProp().asNode(),	Var.alloc(HLCA.endTimeSparqlVar));
//		Triple pattern2 = Triple.create(Var.alloc(HLCA.llcSparqlVar), ont.getEndTimeProp().asNode(),	Var.alloc(HLCA.endTimeSparqlVar));
...
//		System.out.println("block 6  " + block);
	...
	//DUPLICATING TO MAKE A ONE UNION FORM
	
	
//	private static ElementTriplesBlock createTriplesBlockForLlcUserStartTime(Resource user, ContextOntology ont) {
	private static ElementUnion createTriplesUnionForLlcUserStartTime(Resource user, ContextOntology ont) { //Asif MMV2.5
...
//		Triple pattern2= Triple.create(Var.alloc(HLCA.llcSparqlVar), ont.getContextOfProp().asNode(), user.asNode());
...
//		System.out.println(union);
	 ...
	 *         and the end time.
	 */
	//CHANGED TO MAKE IN A UNION SHAPE
	
//	private static ElementTriplesBlock createTriplesBlockForLlcUserStartAndEndTime(Resource user, ContextOntology ont) {
	private static ElementUnion createTriplesUnionForLlcUserStartAndEndTime(Resource user, ContextOntology ont) {//Asif MMV2.5

//		ElementTriplesBlock block = createTriplesBlockForLlcStartAndEndTime(ont);
		ElementUnion union = createTriplesUnionForLlcStartAndEndTime(ont); //Asif MMV2.5
...
//	private static ElementTriplesBlock createTriplesBlockForLlcUserAndEndTimeVar(ContextOntology ont) {
	private static ElementUnion createTriplesUnionForLlcUserAndEndTimeVar(ContextOntology ont) { //Asif MMV2.5
...
//System.out.println("PatternValues block 5");
	 ...
	 */
// private static Query createHlcQuery(ElementTriplesBlock block, ElementFilter filter1, ElementFilter filter2) {
	private static Query createHlcQuery(ElementUnion union, ElementFilter filter1, ElementFilter filter2) { //asif MMV2.5
	...
	//private static Query createLlcQuery(ElementTriplesBlock block, ElementFilter filter1, ElementFilter filter2) {
		...
		private static Query createLlcQuery(ElementUnion union, ElementFilter filter1, ElementFilter filter2) { //MMV2.5
		ElementGroup body = new ElementGroup();
		body.addElement(union);
		body.addElement(filter1);
		body.addElement(filter2);
	 ...
	 */
//	private static Query createMinStartTimeQuery(ElementTriplesBlock block, ElementFilter filter) {
	private static Query createMinStartTimeQuery(ElementUnion union , ElementFilter filter) { //Asif MMV2.5
