...
import org.uclab.scl.framework.RecInterpreter.InterpreterPacket;
import org.uclab.scl.framework.RecInterpreter.RecInterpreter;
...
public class SCLServiceResource {
  ...
  public SCLResponse buildSituationBasedPARecommendation(SituationNotification situationNotification) {
    ...
    try {
      ...
      foodLog.setUserId(Long.parseLong(situationNotification.getUserID()));
      LOG.debug("call for food log");
      List<FoodLog> foodLogList = dclManager.getFoodLog(foodLog);
      LOG.debug("food log received!");
      long totalFat = 0;
      long totalCorbs = 0;
      long totalProts = 0;
      ...
      for (FoodLog food : foodLogList) {
        ...
        totalCorbs += food.getTotalCarbohydrate();
        totalProts += food.getTotalProtein();
      }
      foodLog.setTotalFat(totalFat);
      foodLog.setTotalCarbohydrate(totalCorbs);
      foodLog.setTotalProtein(totalProts);
      ...
      recIngredients.setFoodLog(foodLog);
      ...
      InterpretationIngredients interpretationIngredients = new InterpretationIngredients();
      interpretationIngredients.setFiredRule(rules);
      interpretationIngredients.setUserID(situationNotification.getUserID());
      RecInterpreter recInterpreter = new RecInterpreter();
      InterpreterPacket interpreterPacket = new InterpreterPacket();
      
      interpreterPacket.setRecommendationInput(interpretationIngredients);
      recInterpreter.interpretRecommendation(interpreterPacket);
      

      situationValues = "";

      if (!(interpreterPacket.getRecommendationOutput().isEmpty())) {
        paRecommendation.setRecommendationDescription(interpreterPacket.getRecommendationOutput().get(0).getFoodDescription());
        paRecommendation.setSituationID(Long.parseLong(situationNotification.getSituationEvent().getSituationID()));
        paRecommendation.setRecommendationDate(currentDate);
        for (SituationConditions sc : situationNotification.getSituationEvent().getListSConditions()) {
          situationValues = situationValues + sc.getConditionValue() + " ";
        }
        paRecommendation.setRecommendationIdentifier(situationValues);
        paRecommendation.setRecommendationTypeID(1);
        paRecommendation.setRecommendationLevelID(1);
        paRecommendation.setRecommendationStatusID(1);
        paRecommendation.setConditionValue(interpreterPacket.getRecommendationOutput().get(0).getRecURL());

        paRecommendations.add(paRecommendation);

        result.add("\n********************************\nRecommendation Interpreter\n********************************\n" + interpreterPacket.getRecommendationOutput().get(0).getRecDescription());
        result.add(interpreterPacket.getRecommendationOutput().get(0).getRecURL());
        result.add(interpreterPacket.getRecommendationOutput().get(0).getCurrentContext());
        result.add(situationValues);

        LOG.debug("Interpreted Recommendations: "
            + interpreterPacket.getRecommendationOutput().get(0).getRecDescription() + "\n"
            + interpreterPacket.getRecommendationOutput().get(0).getRecURL()
            + "************End********\n");
      }
  ...
  /*
   * 
   * Testing WebServices Contracts
   */

  @Path("SNSFoodTrendTest/{foodCatagory}")
  @GET
  @Produces("application/json")
  @Consumes("application/json")
  public SNSTrends getSNSTrendsTest(@PathParam("foodCatagory") String foodCatagory) {
    return dclManager.getSNSTrendsFromSLforFoods(foodCatagory);
  }
